...
async function inicializar() {
    ...
    await carregarQuizzesDosModulos();
    ...
    setTimeout(() => {
        showToast(`Bem-vindo ${cursoState.usuario_nome} ao curso de ${cursoState.curso_info.titulo}!`, 'info');
    }, 1500);
}



// --- Sistema de Perguntas Dinâmico do Banco de Dados ---
let perguntasDB = {
    'final': [] // Inicializa vazio, será carregado dinamicamente
};

// Função para carregar perguntas da prova final do banco de dados
async function carregarProvaFinalDoBanco() {
    try {
        console.log('📥 Carregando prova final do banco de dados...');
        
        const response = await fetch(`ajax/carregar_prova_final.php?curso_id=${cursoState.curso_id}`);
        
        if (!response.ok) {
            throw new Error('Erro ao carregar prova final');
        }
        
        const data = await response.json();
        console.log('📊 Dados da prova final recebidos:', data);
        
        if (data.success && data.prova_final && data.perguntas) {
            // Formatar perguntas no formato esperado pelo sistema
            perguntasDB.final = data.perguntas.map(pergunta => ({
                p: pergunta.pergunta,
                o: pergunta.opcoes,
                r: pergunta.resposta_correta,
                explicacao: pergunta.explicacao || ''
            }));
            
            console.log(`✅ Prova final carregada: ${perguntasDB.final.length} perguntas`);
            return true;
        } else {
            console.warn('⚠️ Nenhuma prova final encontrada no banco');
            return false;
        }
    } catch (error) {
        console.error('❌ Erro ao carregar prova final:', error);
        
        // Fallback para perguntas padrão em caso de erro
        perguntasDB.final = [
            { 
...
// Função para carregar quizzes dos módulos do banco de dados
async function carregarQuizzesDosModulos() {
    try {
        console.log('📥 Carregando quizzes dos módulos...');
        
        const response = await fetch(`ajax/carregar_quizzes_modulos.php?curso_id=${cursoState.curso_id}`);
        ...
        const data = await response.json();
        console.log('📊 Dados dos quizzes recebidos:', data);
        
        if (data.success && data.quizzes) {
            // Adicionar quizzes de cada módulo
            data.quizzes.forEach(quiz => {
                if (quiz.modulo_id && quiz.perguntas && quiz.perguntas.length > 0) {
                    perguntasDB[quiz.modulo_id] = quiz.perguntas.map(pergunta => ({
                        p: pergunta.pergunta,
                        o: pergunta.opcoes,
                        r: pergunta.resposta_correta,
                        explicacao: pergunta.explicacao || ''
                    ...
                    console.log(`✅ Quiz do módulo ${quiz.modulo_id} carregado: ${perguntasDB[quiz.modulo_id].length} perguntas`);
                }
            });
            
            return true;
        } else {
            console.warn('⚠️ Nenhum quiz encontrado no banco');
            return false;
        }
    } catch (error) {
...
// Função modificada para carregar a prova final
async function carregarProvaFinal() {
    ...
    const carregouProva = await carregarProvaFinalDoBanco();
    
    if (!carregouProva || perguntasDB.final.length === 0) {
        showToast('❌ Prova final não configurada ou sem perguntas.', 'error');
        carregarDashboard();
        return;
    }
    ...
    const provaInfo = document.createElement('div');
    provaInfo.className = 'prova-info';
    provaInfo.innerHTML = `
        <div class="prova-meta">
            <span><i class="fas fa-clock"></i> Tempo estimado: ${Math.ceil(perguntasDB.final.length * 1.5)} minutos</span>
        ...
        <div class="quiz-instructions">
            <h4>Instruções:</h4>
            <ul>
                <li>Responda todas as ${perguntasDB.final.length} questões</li>
                <li>Você precisa de 70% de acertos para aprovação</li>
                <li>Tempo estimado: ${Math.ceil(perguntasDB.final.length * 1.5)} minutos</li>
                <li>Leia cada questão com atenção antes de responder</li>
            </ul>
        </div>
    `;
    
    provaTemplate.querySelector('p').after(provaInfo);
    
    // Renderizar perguntas reais do banco
    ...
    const btnFinalizar = moduloViewContainer.querySelector('#btn-finalizar-prova');
    if (btnFinalizar) {
        btnFinalizar.addEventListener('click', () => finalizarProva(btnFinalizar));
    }
}

// Função modificada para carregar quiz do módulo
function carregarQuizModulo(idModulo, container) {
    ...
    // Verificar se existem perguntas para este módulo
    const perguntasModulo = perguntasDB[idModulo] || [];
    
    if (perguntasModulo.length === 0) {
        container.innerHTML = `
            <div class="alert alert-warning">
                <i class="fas fa-exclamation-triangle"></i>
                Quiz não configurado para este módulo.
            </div>
        `;
        return;
    }
    ...
    if (quizInfo) {
        ...
        if (quizStats) {
            quizStats.innerHTML = `
                <span><i class="fas fa-question-circle"></i> ${perguntasModulo.length} pergunta(s)</span>
                <span><i class="fas fa-trophy"></i> Complete para avançar</span>
            `;
        }
    }
    
    renderizarPerguntas(perguntasModulo, perguntasWrapper, idModulo);
    quizTemplate.querySelector('.btn-finalizar-quiz').dataset.moduloId = idModulo;
    container.innerHTML = quizTemplate.innerHTML;
    
    // Adicionar evento ao botão de finalizar
    const btnFinalizar = container.querySelector('.btn-finalizar-quiz');
    if (btnFinalizar) {
        btnFinalizar.addEventListener('click', () => {
            finalizarQuizModulo(idModulo, btnFinalizar);
        });
    }
}
...
async function finalizarQuizModulo(idModulo, btn) {
    ...
    const perguntasModulo = perguntasDB[idModulo] || [];
    const { acertos, total, detalhes } = verificarAcertos(idModulo, perguntasModulo);
...
function verificarAcertos(id, perguntasCustom = null) {
    const perguntas = perguntasCustom || perguntasDB[id] || [];
    let acertos = 0;
    const container = moduloViewContainer;
    const detalhes = {
        acertos: 0,
...
// Função para finalizar quiz do módulo
async function finalizarQuizModulo(idModulo, btn) {
    console.log('=== FINALIZANDO QUIZ DO MÓDULO ===');
...
function mostrarResultadoQuizModulo(moduloId, percentual, acertos, total, resultados, aprovado, quizData) {
    const resultadoDiv = document.getElementById(`resultado-quiz-${moduloId}`);
    const form = document.getElementById(`quiz-form-${moduloId}`);
    ...
    resultados.forEach((resultado, index) => {
        detalhesHTML += `
            <div class="pergunta-resultado ${resultado.acertou ? 'acerto' : 'erro'}">
                ...
                <div class="resposta-info">
                    <div class="resposta-usuario">
                        <strong>Sua resposta:</strong> ${resultado.respostaUsuario !== null ? resultado.opcoes[resultado.respostaUsuario] : 'Não respondida'}
                    </div>
                    ${!resultado.acertou ? `
                        <div class="resposta-correta">
                            <strong>Resposta correta:</strong> ${resultado.opcoes[resultado.respostaCorreta]}
                    ...
                    ${resultado.explicacao ? `
                        <div class="explicacao">
                            <strong>Explicação:</strong> ${resultado.explicacao}
                        </div>
                    ` : ''}
                </div>
            </div>
    ...
    resultadoDiv.innerHTML = `
        <div class="resultado-quiz ${aprovado ? '' : 'reprovado'}">
            <div class="resultado-titulo">
                ${aprovado ? '🎉 Parabéns! Quiz Aprovado!' : '📝 Quiz Não Aprovado'}
            </div>
            <div class="resultado-nota">${percentual}%</div>
            <div class="resultado-mensagem">
                ${aprovado ? 
                    `Você acertou ${acertos} de ${total} questões e atingiu a nota mínima de ${quizData.nota_minima}%.` :
                    `Você acertou ${acertos} de ${total} questões. É necessário ${quizData.nota_minima}% para aprovação.`
                }
            </div>
            ...
            <div class="acoes-resultado">
                <button class="btn ${aprovado ? 'btn-success' : 'btn-primary'}" onclick="carregarModulo(${moduloId})">
                    <i class="fas fa-arrow-left"></i> Voltar ao Módulo
                </button>
                ${!aprovado ? `
                    <button class="btn btn-warning" onclick="reiniciarQuizModulo(${moduloId})">
                        <i class="fas fa-redo"></i> Tentar Novamente
                    </button>
                ` : ''}
            </div>
...
function reiniciarQuizModulo(moduloId) {
    const resultadoDiv = document.getElementById(`resultado-quiz-${moduloId}`);
    const form = document.getElementById(`quiz-form-${moduloId}`);
